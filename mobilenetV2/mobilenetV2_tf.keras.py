# 通过tf.keras实现自定义的mobilenetv2网络并通过tf.keras训练
import os
import time
import random
import shutil
...
class MyCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs: dict):
        print("On train end (epuch: %d, lr: %0.5f)" % (epoch+1, logs["lr"]))
        print("")
        tmp = "./tmp/"
        if epoch == 0 and os.path.exists(tmp):
            shutil.rmtree(tmp)                   # 删除现有路径，os.rmdir(tmp)函数只能用于删除空目录
            os.makedirs(tmp)                     # 新建立路径
        localtime = time.strftime("%Y%m%d-%H%M", time.localtime())
        output_model = tmp + "{}_mobilenetv2-v0.4_{}_{}_{}_{}_{:.4f}_{:.4f}.h5".format(dataset_name, localtime, IMG_SIZE[0], alpha, epoch+1, logs["sparse_categorical_accuracy"], logs["val_sparse_categorical_accuracy"])
...
# 编译模型，这里如果metrics设置为accuracy，则存在一个bug，在低版本tf，当loss是sparse_categorical_crossentropy,metrics是
# accuracy时，存在load模型后，采用evaluate评价函数，acc不正确的bug.
...
# 训练模型，只是训练最后一层，冻结前面所有层；回调函数的顺序有影响，需要先学习率再自定义的，这样logs里才有lr
history = model.fit(train_dataset, epochs=initial_epochs, validation_data=test_dataset, verbose=2, callbacks=[learningreatescheduler, MyCallback()])
# print(model.get_layer(name="block_8_project_BN").get_weights()[0][:4])
...
# 小一点以及滑动系数设置的小一点。
loss1, accuracy1 = model.evaluate(test_dataset, verbose=2)
...
# 重新编译模型，降低学习率
model.compile(optimizer = Adam(learning_rate=second_learning_rate),
                loss='sparse_categorical_crossentropy',
                metrics=['sparse_categorical_accuracy'])

# 开始第二轮训练；回调函数的顺序有影响，需要先学习率再自定义的，这样logs里才有lr
total_epochs =  initial_epochs + second_epochs
history_fine = model.fit(train_dataset, epochs=total_epochs, initial_epoch=history.epoch[-1]+1, validation_data=test_dataset, 
                                                                verbose=2, callbacks=[learningreatescheduler, MyCallback()])

# 测试输出模型准确率
loss2, accuracy2 = model.evaluate(test_dataset, verbose=2)
print("last loss: {:.4f}".format(loss2))
print("last accuracy: {:.4f}".format(accuracy2))

# 保存模型参数和结构
localtime = time.strftime("%Y%m%d-%H%M", time.localtime())
output_model = output_path + "{}_mobilenetv2-v0.4_{}_{}_{}_{}_{:.4f}_{:.4f}.h5".format(dataset_name, localtime, IMG_SIZE[0], alpha, total_epochs, history_fine.history["sparse_categorical_accuracy"][-1], accuracy2)
model.save(output_model)
